...
import { reviewPullRequestFromUrl } from '../../../../plus/launchpad/utils/-webview/reviewPr.utils.js';
...
export class CliCommandHandlers implements Disposable {
	...
	@command('compose')
	async handleComposeCommand(request: CliCommandRequest, repo?: Repository | undefined): Promise<CliCommandResponse> {
		if (!repo || !request.args?.length) {
	...
	@command('review-pr')
	async handleReviewPrCommand(
		request: CliCommandRequest,
		_repo?: Repository | undefined,
	): Promise<CliCommandResponse> {
		if (!request?.args?.length) return { stderr: 'No PR URL provided' };
		const [prUrl] = request.args;

		try {
			const { worktree, branch, pr } = await reviewPullRequestFromUrl(this.container, prUrl);
